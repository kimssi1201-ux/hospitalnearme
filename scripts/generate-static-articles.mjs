...
function eventGalleryMarkup(title, images) {
  if (!images.length) return "";
  return `<section class="event-gallery" aria-labelledby="galleryTitle"><p class="eyebrow">FESTIVAL PHOTOS</p><h2 id="galleryTitle">축제 사진</h2><div class="event-gallery-grid">${images.map((image, index) => `<figure><img src="${escapeHtml(image)}" alt="${escapeHtml(title)} 추가 사진 ${index + 1}" loading="lazy" /></figure>`).join("")}</div></section>`;
}
...
function eventPage(event, sourceLabel) {
  ...
  const image = officialImage(event.image);
  const galleryImages = [...new Set((Array.isArray(event.galleryImages) ? event.galleryImages : []).map(officialImage).filter(Boolean))]
  ...
  const jsonLd = JSON.stringify({
    ...
    name: title,
    description,
    image: image || galleryImages.length ? [image, ...galleryImages].filter(Boolean) : undefined,
